...
def delete_task(task_id: int, user_id: UUID = Depends(verify_jwt_token), session: Session = Depends(get_session)):
    ...
    # First, check if the taskhistory table exists before attempting to delete from it
    ...
    if table_exists:
        try:
            # Attempt to delete related task history records if the table exists
            session.execute(text("DELETE FROM taskhistory WHERE task_id = :task_id"), {"task_id": task_id})
        except Exception:
            # If there's an issue with the delete operation, continue with task deletion
            pass
